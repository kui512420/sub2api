...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	//   - ShouldAccumulateUserPlatformQuota 守卫:白名单外平台（如原生上游 jiaotu）不可能有配额行，跳过
	...
	if !p.IsSubscriptionBill && cost.ActualCost > 0 && p.User != nil && deps.userPlatformQuotaRepo != nil &&
		ShouldAccumulateUserPlatformQuota(p.Platform) {
...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	//   - ShouldAccumulateUserPlatformQuota 守卫:白名单外平台（如 jiaotu）直接跳过，否则每次请求一条
	//     user_platform_quotas_platform_check 违反的 ALERT，且 flusher 的多行 UPSERT 会被这一行整体打断
	...
	if !p.IsSubscriptionBill && p.Cost.ActualCost > 0 && p.User != nil && deps.userPlatformQuotaRepo != nil &&
		ShouldAccumulateUserPlatformQuota(p.Platform) {
