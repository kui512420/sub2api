export default {
    accounts: {
      ...
      // Jiaotu native upstream: account form / pool import / points display
      jiaotu: {
        platformLabel: 'Jiaotu',
        tokenLabel: 'Jiaotu Token',
        tokenPlaceholder: 'Paste the upstream JWT (Bearer prefix allowed)',
        tokenHint: 'The backend only reads credentials.token; the token cannot be refreshed, so 401/403 marks the account expired.',
        tokenRequired: 'Please enter the Jiaotu token',
        phoneLabel: 'Phone (optional)',
        phonePlaceholder: '13800000000',
        poolIdLabel: 'Pool ID (optional)',
        poolIdPlaceholder: 'e.g. jp-9',
        poolIdHint: 'Dedup key for pool imports. Keep it so re-importing the same pool does not create duplicate accounts.',
        nickNameLabel: 'Upstream nickname (optional)',
        pointsLabel: 'Points snapshot (optional)',
        pointsPlaceholder: 'e.g. 68',
        pointsHint: 'Display only; the authoritative balance comes from the free userBilling/page query refreshed by pool maintenance.',
        singleAccountConcurrencyHint: 'Keep concurrency at 1: Jiaotu spends points per account, and concurrent load on one account easily trips upstream risk control.',
        points: 'Points',
        noPointsSnapshot: 'No snapshot',
        statusExpired: 'Token expired',
        statusError: 'Upstream error',
        statusOk: 'Healthy',
        poolImport: {
          title: 'Import Jiaotu Pool',
          hint: 'Paste kuikui server/data/.jiaotu-pool.json (also accepts {code,data} status payloads, bare arrays, or one token per line).',
          warning: 'Imports dedup by pool ID + token; accounts whose upstream status is not ok get created but immediately disabled.',
          contentLabel: 'Pool content',
          contentPlaceholder: '{"accounts":[{"id":"jp-9","token":"…","phone":"…","points":49}]}',
          namePrefixLabel: 'Account name prefix (optional)',
          namePrefixPlaceholder: 'Defaults to Jiaotu',
          groupLabel: 'Bind groups',
          groupHint: 'Groups must be platform=jiaotu with image/video capability enabled; leave empty to skip binding.',
          proxyLabel: 'Proxy (optional)',
          concurrencyLabel: 'Per-account concurrency',
          priorityLabel: 'Scheduling priority',
          updateExisting: 'Refresh credentials and points for existing accounts',
          submit: 'Start Import',
          importing: 'Importing…',
          emptyContent: 'Paste the pool content first',
          success: 'Import finished: {created} created, {updated} updated, {skipped} skipped, {failed} failed',
          failed: 'Jiaotu pool import failed',
          resultSummary: 'Result: {total} entries, {ready} usable, {unusable} unusable',
          targetSize: 'Pool target size: {count}',
          itemCreated: 'created',
          itemUpdated: 'updated',
          itemSkipped: 'skipped',
          itemFailed: 'failed',
        ...
        maintenance: {
          ...
          hint: 'Refreshes pool-wide points (free userBilling/page calls only, spends no points) and optionally runs the daily free check-in.',
          refreshPoints: 'Refresh points',
          signIn: 'Daily check-in',
          concurrencyLabel: 'Concurrency (max 10)',
          limitLabel: 'Max accounts to process (0 = whole pool)',
          run: 'Start',
          running: 'Running…',
          success: 'Maintenance finished: {refreshed} refreshed, {unchanged} unchanged, {failed} failed, {expired} expired',
          failed: 'Jiaotu pool maintenance failed',
          signInSummary: 'Check-in: {success} ok, {already} already done, {failed} failed'
        }
      },
      cnProviders: {
        accountMode: {
