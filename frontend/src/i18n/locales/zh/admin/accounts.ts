export default {
    accounts: {
      ...
      // 椒图（Jiaotu）原生上游：账号表单 / 号池导入 / 积分展示
      jiaotu: {
        platformLabel: '椒图',
        tokenLabel: '椒图 Token',
        tokenPlaceholder: '粘贴上游 JWT（可带 Bearer 前缀）',
        tokenHint: '后端只读 credentials.token；token 无 refresh 能力，401/403 会把账号置为 expired。',
        tokenRequired: '请填写椒图 token',
        phoneLabel: '手机号（可选）',
        phonePlaceholder: '13800000000',
        poolIdLabel: '号池 ID（可选）',
        poolIdPlaceholder: '如 jp-9',
        poolIdHint: '批量导入的去重键；手填时建议一并写下，方便以后重复导入不产生第二份账号。',
        nickNameLabel: '上游昵称（可选）',
        pointsLabel: '积分快照（可选）',
        pointsPlaceholder: '如 68',
        pointsHint: '仅展示用；实际积分以免费的 userBilling/page 查询为准（在号池维护里刷新）。',
        singleAccountConcurrencyHint: '建议并发保持 1：椒图按号计积分，单号并发打高容易触发上游风控。',
        points: '积分',
        noPointsSnapshot: '未刷新',
        statusExpired: '登录态失效',
        statusError: '上游报错',
        statusOk: '正常',
        poolImport: {
          title: '导入椒图号池',
          hint: '粘贴 kuikui 的 server/data/.jiaotu-pool.json（也支持 {code,data} 状态包、裸数组、每行一个 token 的纯文本）。',
          warning: '导入按「号池 ID + token」双键去重；上游状态非 ok 的号会被直接下架。',
          contentLabel: '号池内容',
          contentPlaceholder: '{"accounts":[{"id":"jp-9","token":"…","phone":"…","points":49}]}',
          namePrefixLabel: '账号名前缀（可选）',
          namePrefixPlaceholder: '默认 椒图',
          groupLabel: '绑定分组',
          groupHint: '分组需 platform=jiaotu 且开启图片/视频能力；留空则不绑定。',
          proxyLabel: '代理（可选）',
          concurrencyLabel: '单号并发',
          priorityLabel: '调度优先级',
          updateExisting: '已存在时也刷新凭据与积分',
          submit: '开始导入',
          importing: '导入中…',
          emptyContent: '请先粘贴号池内容',
          success: '导入完成：新增 {created}、更新 {updated}、跳过 {skipped}、失败 {failed}',
          failed: '椒图号池导入失败',
          resultSummary: '结果：共 {total} 条，可用 {ready}，不可用 {unusable}',
          targetSize: '号池目标规模 {count}',
          itemCreated: '新增',
          itemUpdated: '更新',
          itemSkipped: '跳过',
          itemFailed: '失败',
          noGroupSelected: '不绑定分组'
        },
        maintenance: {
          title: '号池维护',
          hint: '全池刷新积分（只打免费的 userBilling/page，不消耗积分），可选顺带做每日免费签到。',
          refreshPoints: '刷新积分',
          signIn: '每日签到',
          concurrencyLabel: '并发（≤ 10）',
          limitLabel: '最多处理账号数（0 = 全池）',
          run: '开始维护',
          running: '维护中…',
          success: '维护完成：刷新 {refreshed}、未变 {unchanged}、失败 {failed}、失效 {expired}',
          failed: '椒图号池维护失败',
          signInSummary: '签到：成功 {success}、已签 {already}、失败 {failed}'
        }
      },
      cnProviders: {
        accountMode: {
